...
def main():
    ...
    # ── Inject text_to_motion_attn into GPT sub-configs ──────────────────────
    _text_to_motion_attn = getattr(config, 'text_to_motion_attn', False)
    if hasattr(config, 'structure_generate'):
        if hasattr(config.structure_generate, 'base'):
            config.structure_generate.base.text_to_motion_attn = _text_to_motion_attn
        if hasattr(config.structure_generate, 'head'):
            config.structure_generate.head.text_to_motion_attn = _text_to_motion_attn
    print(f'[text_to_motion_attn] {_text_to_motion_attn}')
