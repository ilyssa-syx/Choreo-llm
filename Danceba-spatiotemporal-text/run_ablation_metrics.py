...
import metrics_new_aist as mn     # noqa: E402  (after chdir)

# ── CLI ──────────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser()
parser.add_argument(
    '--gt_root',
    default=os.path.join(UTILS_DIR, '../data/aist_features_zero_start'),
    help='Ground-truth feature root (默认 data/aist_features_zero_start，相对于 utils/)'
)
parser.add_argument(
    '--ablations',
    nargs='+',
...
parser.add_argument(
    '--results_json',
    default=os.path.join(SCRIPT_DIR, 'ablation_metrics_results.json'),
    help='增量保存的 JSON 文件路径'
)
parser.add_argument(
    '--plot_out',
    default=os.path.join(SCRIPT_DIR, 'ablation_metrics_plot.png'),
    help='折线图保存路径'
)
